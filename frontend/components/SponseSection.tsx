...
const SponsorSection: React.FC = () => {
  ...
  const handleSponsorChange = (
    ...
    setNewSponsor((prevData: any) => ({ ...prevData, [name]: value }));
  };
  ...
  const addSponsor = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setTimeout(() => {
      if (editIndex !== null) {
        ...
        setSponsors((prevSponsors: any) => [...prevSponsors, newSponsor]);
      }
      setNewSponsor({});
      setIsAdding(false);
    }, 1000);
  ...
  const handleDelete = (index: number) => {
    setSponsors((prevSponsors: any) => prevSponsors.filter((_, i:any) => i !== index));
    setNewSponsor({});
  };
